...
export const TaskForm = ({ onSaveTask, onCancel }) => {
  const [details, setDetails] = useState({title: '', description: '', dueDate: ''})
  ...
  return (
    <form onSubmit={submitHandler} className="bg-white p-6 rounded-md shadow-md max-w-lg mx-auto" onChange={changeHandler}>
      ...
      <div className="mb-4">
        ...
        <input type="date" id="dueDate" name="dueDate" value={details.dueDate} className="w-full p-2 border border-gray-300 rounded-md" required />
      </div>
